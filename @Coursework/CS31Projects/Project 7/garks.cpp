...
Gark::Gark(Mesa* mp, int r, int c)
{
    ...
    m_row = r;
    m_col = c;
    m_attacked = false; //initialized additional data type for attack behavior
}
...
bool Gark::getAttacked(int dir)  // return true if dies
{
    if (m_attacked)
        return true;
    bool inMesa = m_mesa->determineNewPosition(m_row,m_col,dir); //only moves if new position is inMesa
    if (inMesa) {
        m_attacked = true;
        return false; //return false since gark moved
    }
    return true;  // doesn't move but gark dies by stepping off
}
...
void Mesa::display() const
{
    ...
    for (r = 0; r < rows(); r++) {
        for (c = 0; c < cols(); c++) {
            ...
            int numGarks = numGarksAt(r + 1,c + 1);
            if (numGarks == 1)
                grid[r][c] = 'G';
            else if (numGarks >= 2 && numGarks <= 8)
                grid[r][c] = '0' + numGarks;
            else if (numGarks >= 9)
                grid[r][c] = '9';
        }
