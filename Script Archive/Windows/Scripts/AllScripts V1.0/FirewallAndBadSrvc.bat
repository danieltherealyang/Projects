@TITLE M360_FirewallAndBadSrvc Script
@ECHO OFF

ECHO THIS SCRIPT IS THE PROPERTY OF M360

ECHO --------------------------------------------------
ECHO M360_FirewallAndBadSrvc Script
...
ECHO 20, 21, 22, 23, 24, 25, 135, 389, 411, 412, 3389
ECHO This script will also [attempt to] disable: Telnet, Microsoft FTP, Telephony, Remote Access, Remote Desktop, Remote Procedure Call (RPC), RIP Listener, SNMP, and some related services.
ECHO --------------------------------------------------
...
:: Port 3389 ::
ECHO Attempting to block port 3389 (TCP/UDP ; IN/OUT)...
netsh advfirewall firewall add rule name="Block 3389 TCP IN" protocol=TCP dir=IN localport=3389 action=BLOCK
netsh advfirewall firewall add rule name="Block 3389 UDP IN" protocol=UDP dir=IN localport=3389 action=BLOCK
netsh advfirewall firewall add rule name="Block 3389 TCP OUT" protocol=TCP dir=OUT localport=3389 action=BLOCK
netsh advfirewall firewall add rule name="Block 3389 UDP OUT" protocol=UDP dir=OUT localport=3389 action=BLOCK
ECHO Finished attempt at blocking 3389 (TCP/UDP ; IN/OUT).
...
ECHO Attempting to disable Remote Desktop Configuration...
sc stop "SESSIONENV"
sc config "SESSIONENV" start= disabled
ECHO Finished attempt at disabling Remote Desktop Configuration

:: Remote Desktop Services ::
ECHO Attempting to disable Remote Desktop Services...
sc stop "TERMSERVICE"
sc config "TERMSERVICE" start= disabled
ECHO Finished attempt at disabling Remote Desktop Services

:: Remote Desktop Services UserMode Port Redirector ::
ECHO Attempting to disable Remote Desktop Services UserMode Port Redirector...
sc stop "UMRDPSERVICE"
sc config "UMRDPSERVICE" start= disabled
ECHO Finished attempt at disabling Remote Desktop Services UserMode Port Redirector
